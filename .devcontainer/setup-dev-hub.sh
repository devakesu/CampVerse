...
# ==============================================================================
# 4.5. Go SDK Setup (For CampVerse Realtime Engine)
# ==============================================================================
GO_HOME="${SDKS_DIR}/go"
...
if [ "${ENABLE_GO}" = "true" ]; then
    ...
    if [ ! -d "${GO_HOME}/bin" ] || [ "${UPDATE_ALL}" = "true" ]; then
...
# ==============================================================================
# 8. Permissions Normalization & Ownership Recovery
# ==============================================================================
echo "🔐 Normalizing permissions across ${DEV_HUB_ROOT}..."

# Reclaim ownership from any root-created files/caches
if [ "$(id -u)" -ne 0 ]; then
    sudo chown -R "${USER}:${USER}" "${DEV_HUB_ROOT}" 2>/dev/null || true
fi

# Clean up npm cache locks if present
if [ -d "${CACHES_DIR}/npm/_cacache" ]; then
    rm -rf "${CACHES_DIR}/npm/_cacache"
fi

# Apply clean permission boundaries
chmod -R 777 "${CACHES_DIR}" 2>/dev/null || true
chmod -R 755 "${SDKS_DIR}" "${BIN_DIR}" 2>/dev/null || true
